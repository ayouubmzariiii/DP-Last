...
interface DossierMeta {
    ...
    empty?: boolean
    summary?: { applicant: string; address: string; worksType: string; files: DossierFiles; abf?: boolean; photo?: string | null }
}
interface Account {
    email: string
    createdAt?: string
                                    ...
                                    <div key={d.id} className="dp-card" style={{ padding: '14px 16px', opacity: lc === 'archive' ? .85 : 1 }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                                            <button onClick={() => router.push(`/etape/${d.id}/${openStep(d)}`)} aria-label={`Ouvrir ${d.title}`}
                                                style={{ width: 96, height: 96, flexShrink: 0, padding: 0, borderRadius: 12, overflow: 'hidden', border: '1px solid var(--line)', background: 'var(--surface-2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 6px -2px rgba(37,34,30,.12)' }}>
                                                {d.summary?.photo ? (
